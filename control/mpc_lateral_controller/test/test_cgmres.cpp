...
class CGMRESTest : public ::testing::Test
{
protected:
  MPCParam param;
...
TEST_F(CGMRESTest, InitializeAndCalculate)
{
  auto node = rclcpp::Node("cgmres_test_node", rclcpp::NodeOptions{});
...
// TEST_F(CGMRESTEST, InitializeAndCalculateRightTurn)
// {
...
//   std::make_shared<QPSolverEigenLeastSquareLLT>(); mpc->setQPSolver(qpsolver_ptr);
//   ASSERT_TRUE(mpc->hasQPSolver());
...
// TEST_F(CGMRESTEST, OsqpCalculate)
// {
//   auto node = rclcpp::Node("mpc_test_node", makeNodeOptions());
//   auto mpc = std::make_unique<MPC>(node);
//   initializeMPC(*mpc);
//   const auto current_kinematics = makeOdometry(dummy_straight_trajectory.points.front().pose,
//   0.0); mpc->setReferenceTrajectory(dummy_straight_trajectory, trajectory_param,
//   current_kinematics);
...
// TEST_F(CGMRESTEST, OsqpCalculateRightTurn)
// {
//   auto node = rclcpp::Node("mpc_test_node", makeNodeOptions());
//   auto mpc = std::make_unique<MPC>(node);
//   initializeMPC(*mpc);
...
// TEST_F(CGMRESTEST, KinematicsNoDelayCalculate)
// {
//   auto node = rclcpp::Node("mpc_test_node", makeNodeOptions());
//   auto mpc = std::make_unique<MPC>(node);
//   initializeMPC(*mpc);
...
// TEST_F(CGMRESTEST, KinematicsNoDelayCalculateRightTurn)
// {
//   auto node = rclcpp::Node("mpc_test_node", makeNodeOptions());
//   auto mpc = std::make_unique<MPC>(node);
//   initializeMPC(*mpc);
...
// TEST_F(CGMRESTEST, DynamicCalculate)
// {
//   auto node = rclcpp::Node("mpc_test_node", makeNodeOptions());
//   auto mpc = std::make_unique<MPC>(node);
//   initializeMPC(*mpc);
...
// TEST_F(CGMRESTEST, MultiSolveWithBuffer)
// {
//   auto node = rclcpp::Node("mpc_test_node", makeNodeOptions());
//   auto mpc = std::make_unique<MPC>(node);
//   std::shared_ptr<QPSolverInterface> qpsolver_ptr =
...
// TEST_F(CGMRESTEST, FailureCases)
// {
//   auto node = rclcpp::Node("mpc_test_node", makeNodeOptions());
//   auto mpc = std::make_unique<MPC>(node);
//   std::shared_ptr<QPSolverInterface> qpsolver_ptr =
